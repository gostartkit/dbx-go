...
func TestHandleLineShowUserIsRemoved(t *testing.T) {
	t.Parallel()
	...
	exit, runErr := app.handleLine(context.Background(), "show user analytics_ro")
	if runErr == nil {
	...
	if !strings.Contains(runErr.Error(), `unknown show target "user"`) {
...
func TestCLIShowUserIsRemoved(t *testing.T) {
	t.Parallel()

	app, _, _ := newCLIApp(t, "", t.TempDir())
	err := app.Run(context.Background(), []string{"show", "user", "analytics_ro"})
	if err == nil {
		t.Fatalf("expected removed command to fail")
	}
	if !strings.Contains(err.Error(), `unknown show target "user"`) {
